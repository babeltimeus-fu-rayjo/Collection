...
class HostSession {
  ...
  watcherChat(id, msg) {
    ...
    if (!w || !text) return;
    const entry = { seat: -1, name: `${w.name} \u{1F441}`, text, ts: Date.now(), channel: 'all' };
    ...
    for (const x of this.watchers) {
      try {
        x.conn.send({ t: 'chat', ...entry });
      } catch {}
    }
    addChatMsg(entry, false);
  }
...
function renderGame(view, sess) {
  ...
  if (view.observer) {
    ...
    $('#chat-ch-team').classList.remove('hidden');
  }
...
function init() {
  ...
  for (const b of document.querySelectorAll('.btn-jointeam')) {
    b.addEventListener('click', () => {
      if (!session || session.observer) return;
      const ti = Number(b.dataset.team);
